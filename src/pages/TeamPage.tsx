...
export default function TeamPage() {
  ...
  if (loading) return <p className="loading">{t('team.loading')}</p>
  if (!team) return <p className="no-data">{t('team.notFound')}</p>
        ...
        <>
          ...
          <div className="next-match__item team-page__upcoming">
            ...
            <p className="next-match__date">
              {formatDate(locale, upcomingMatch.utcDate, timeZone, {
                weekday: 'short',
                day: 'numeric',
                month: 'short',
              })}
              {' · '}
              {formatTime(locale, upcomingMatch.utcDate, timeZone)}
      ...
      <>
        ...
        <div className="scorers-list">
          {scorers.length > 0 ? (
            ...
            <div>
              <span className="scorer-row__none">{t('team.noScorers')}</span>
            </div>
          )}
        </div>
      </>
        ...
        <>
          ...
          <div className="coach-card">
            ...
            <div className="coach-card__meta">
              <span>{t('team.nationality')}: {team.coach.nationality ?? t('common.notAvailable')}</span>
              <span>{t('team.born')}: {team.coach.dateOfBirth ? team.coach.dateOfBirth.slice(0, 4) : t('common.notAvailable')}</span>
            </div>
          </div>
        </>
      )}
        ...
        {team.squad?.map(player => (
          <div key={player.id} className="squad-card">
            ...
            <div className="squad-card__meta">
              <span>{player.position ?? t('common.notAvailable')}</span>
              <span>{player.nationality ?? t('common.notAvailable')}</span>
              <span>{player.dateOfBirth ? player.dateOfBirth.slice(0, 4) : t('common.notAvailable')}</span>
            </div>
          </div>
        ))}
      </div>
